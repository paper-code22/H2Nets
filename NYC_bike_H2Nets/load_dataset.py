...
# Bike
def load_bike_dataset(type):
    # bike volume data
    train_data_path = os.path.join(path + '/data/NYC_bike_volume_train.npz')
    test_data_path = os.path.join(path + '/data/NYC_bike_volume_test.npz')
    train_data_ = np.load(open(train_data_path, "rb"))["volume"]#[:,:,:,0]  # pickup or dropoff volume; (1920, 10, 20, 2)
    test_data_ = np.load(open(test_data_path, "rb"))["volume"]#[:,:,:,0]   # pickup dropoff volume; (960, 10, 20, 2)

    train_data_max = 1. # train_data_.max() # 299.
    test_data_max = 1. #test_data_.max() # 307.
...
def load_hyperedge(hyper_data_type):
    # Bike
    # shape of hyperedge is (1, 2, 1109); to compare, the shape of hyperedge of Taxi is (1, 2, 1175)
    hyperedge_path = os.path.join(path + '/data/NYC_bike_hyperedge.npz')
    hyperedge_ = np.load(hyperedge_path)['hyperedge'] # (39, 200) - important information
...
def load_hypernode(hyper_data_type):
    # Bike
    # shape of load_hypernode is ((1, 2, 1099))
    hypernode_path = os.path.join(path + '/data/NYC_bike_hypernode_cos.npz')
    hypernode_ = np.load(hypernode_path, allow_pickle=True)['hypernode'] # (23, 100) - important information
...
def load_Hodge_Laplacian():
    # L1: (M, M)
    hodge_laplacian_path = os.path.join(path + '/data/NYC_bike_Hodge_Laplacian.npz')
...
def load_incidence_matrix():
    # B1: (M, M)
    incidence_matrix_path = os.path.join(path + '/data/NYC_bike_B1.npz')
...
def load_edge_dataset():
    # bike edge data
    train_edge_data_path = os.path.join(path + '/data/NYC_bike_train_x_edge.npz')
    test_edge_data_path = os.path.join(path + '/data/NYC_bike_test_x_edge.npz')
